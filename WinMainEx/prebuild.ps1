# prebuild.ps1 — dev pre-build step for WindowsProject (the Win32X sample app).
#
# WindowsProject registers itself as the exefile "open" DelegateExecute COM handler, so
# rpcss keeps a resident -Embedding server alive that holds WindowsProject.exe open. That
# makes the linker fail with LNK1104 ("cannot open file ... WindowsProject.exe") on every
# rebuild. This script clears that state before each build:
#   1. kill any running WindowsProject instance (frees the exe for the linker)
...
Get-Process WindowsProject -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
